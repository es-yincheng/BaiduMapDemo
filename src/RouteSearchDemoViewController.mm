...
//        self.edgesForExtendedLayout=UIRectEdgeNone;
	...
	_startCityText.text = @"上海";
	_startAddrText.text = @"上海长宁区长宁路1139号";
	_endCityText.text = @"上海";
    _endAddrText.text = @"上海市闸北区恒丰路31号";
...
{
    NSLog(@"返回结果是：%u",error);
//    NSLog(@"start建议地址：%@",result.suggestAddrResult.startPoiList);
    for (int i=0; i<result.suggestAddrResult.startPoiList.count; i++) {
        BMKPoiInfo *info =[result.suggestAddrResult.startPoiList objectAtIndex:i];
        NSLog(@"start (%d) :%@：%f , %f",i,info.address,info.pt.latitude,info.pt.longitude);
    }
    for (int i=0; i<result.suggestAddrResult.endPoiList.count; i++) {
        BMKPoiInfo *info =[result.suggestAddrResult.endPoiList objectAtIndex:i];
        NSLog(@"end (%d) :%@：%f , %f",i,info.address,info.pt.latitude,info.pt.longitude);
    }
...
//            [plan.distance]
//            [transitStep.stepType]
            
//            NSLog(@"%@ ：%@ %@",i,plan.distance)
...
//            @property (nonatomic) int zonePrice;
            ...
            switch (transitStep.stepType) {
                case BMK_BUSLINE:
                    NSLog(@"公交:%d",transitStep.vehicleInfo.passStationNum);
                    NSLog(@"%@,%@",transitStep.entrace.title,transitStep.exit.title);
                ...
                case BMK_SUBWAY:
                    NSLog(@"地铁");
                    NSLog(@"地铁:%d",transitStep.vehicleInfo.passStationNum);
                    NSLog(@"%@,%@",transitStep.entrace.title,transitStep.exit.title);
                    NSLog(@"%@",transitStep.instruction);
                    break;
                case BMK_WAKLING:
...
//            NSLog(@"%ld : %@",i,transitStep.stepType,transitStep.distance,transitStep);
...
//    startpt.latitude = 31.213301;
//    startpt.longitude = 121.387616;
//    start.pt = startpt;
	BMKPlanNode* end = [[BMKPlanNode alloc]init];
	end.name = _endAddrText.text;
//    CLLocationCoordinate2D endpt;
//    endpt.latitude = 31.246058;
//    endpt.longitude = 121.465144;
//    end.pt = endpt;
